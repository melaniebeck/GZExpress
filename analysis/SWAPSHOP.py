...
parser.add_argument("--config", help='Addendum to the config filename')
parser.add_argument("-n", "--new", dest="startup", action="store_true", 
                    default=False, help="start new SWAPSHOP with startup.config")
parser.add_argument("-d", "--dir", dest="directory_name", default=None,
...
if args.directory_name: 
    log_dir = "logfiles_%s"%(args.directory_name)
else: 
    log_dir = "logfiles"
...
while more_to_do:
    ...
    if int(subprocess.check_output("grep 'running' .swap.%s | wc -l"\
                                    %cookie, shell=True)):
        try: config = args.config_name.replace('startup','update')
