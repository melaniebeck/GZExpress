...
# Extract all the classification probabilities used by the agents:
    ...
    def collect_probabilities(self):
        ...
        for ID in self.list():
            ...
            PLarray.append(agent.PL)
            PDarray.append(agent.PD)
            contributions.append(agent.contribution)
            skills.append(agent.skill)
            Ntraining.append(agent.NT)
            Ntotal.append(agent.N)  # Nc

        # GENERISIZE THIS
        self.probabilities['FEAT'] = PLarray
        self.probabilities['NOT'] = PDarray
        self.contributions = contributions
        self.skills = skills
        self.Ntraining = Ntraining
        self.Ntotal = Ntotal
        self.Ntest = np.array(Ntotal) - np.array(Ntraining)
...
# Prepare to plot agents' histories:
    ...
    def finish_history_plot(self,axes,time,filename):

        plt.sca(axes)

        # Timestamp:
        plt.text(100,swap.Imax+0.02,time,color='gray')
...
# element probabilities:
    ...
    def plot_probabilities(self,Nc,t,filename):
        ...
        PL = self.probabilities['FEAT'][index]
        ...
        for label in lowerhist.get_yticklabels():
            label.set_visible(False)
        # GENERISIZE THIS
        lowerhist.set_xlabel('Pr("FEAT"|FEAT)')
