...
#   eg.  sample.member[ID].was_described(by=agent,as_being='LENS',at_time=t)
    ...
    def was_described(self,by=None, as_being=None, at_time=None, 
                      while_ignoring=0, haste=False, at_x=[-1], at_y=[-1], 
                      online=True, record=True, realize_confusion=False, 
                      laplace_smoothing=0, machine=False):
        ...
        # not their *state*.
            ...
            if record and machine:
                # GENERISIZE THIS
                as_being_dict = {'FEAT': 1, 'NOT': 0}
                self.annotationhistory['Name'].append(by.name)
                self.annotationhistory['ItWas'].append(as_being_dict[as_being])
                self.annotationhistory['At_X'].append(0)
                self.annotationhistory['At_Y'].append(0)
                self.annotationhistory['PL'].append(0)
                self.annotationhistory['PD'].append(0)
            ...
            # update the annotation history
                ...
                self.annotationhistory['PL'].append(by.PL)
                self.annotationhistory['PD'].append(by.PD)
                self.annotationhistory['At_Time'].append(at_time)
            ...
            if by.NT > a_few_at_the_start:
                ...
                if online:
                    ...
                    if self.kind == 'test' and record:
                         ...
                         by.testhistory['ID'].append(self.ID)
                         by.testhistory['I'].append(swap.informationGain(\
                                                    self.mean_probability,
                                                    by.PL, by.PD, as_being))
                         by.testhistory['Skill'].append(by.skill)
                         by.testhistory['ItWas'].append(as_being_number)
                         by.testhistory['At_Time'].append(at_time)
...
# Update probability of LENS, given many classifications at once (E step):
    ...
    def was_described_many_times(self, bureau, names, classifications, record=False, haste=False, while_ignoring=-1,realize_confusion=False,laplace_smoothing=0):
        ...
        for name, classification in zip(names, classifications):
            ...
            by = agent
            # GENERISIZE THIS
            as_being = ['NOT', 'FEAT'][classification]
