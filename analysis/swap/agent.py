...
# GENERIZE THIS
actually_it_was_dictionary = {'FEAT': 1, 'NOT': 0, 'UNKNOWN': -1}
    ...
    def heard(self,it_was=None, actually_it_was=None, with_probability=1.0,
        ...
        if it_was==None or actually_it_was==None:
            ...
            if record:
                ...
                self.traininghistory['ID'].append(ID)
                # Always log progress, even if not learning:
                self.traininghistory['Skill'].append(self.update_skill())
                # NB. self.skill is now up to date.
                self.traininghistory['PL'].append(self.PL)
                self.traininghistory['PD'].append(self.PD)
                self.traininghistory['ItWas'].append(actually_it_was_dictionary[it_was])
                self.traininghistory['ActuallyItWas'].append(\
